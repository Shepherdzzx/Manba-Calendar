...
本文定义 Manba Alert 第一版中，大模型输出给应用侧的结构化 JSON 指令格式。

目标是让模型输出稳定、应用可校验、关键操作可确认。当前 v1 只覆盖三个高频动作：
...
- 应用侧**不能直接信任模型输出**，必须先校验后执行
- 对删除类操作，应用侧应增加确认步骤，不能仅凭模型结果直接执行
