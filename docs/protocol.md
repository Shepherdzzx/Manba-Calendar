...
- 当用户使用“今天的事”“今天的安排”这类泛化表达时，`title` 可以为空，此时执行器应按 `date`（及可选 `time`）查找候选事件
- 如果按日期范围匹配到多个事件，执行器必须返回冲突，而不是直接删除
- 当按 `date` / `time` 命中多个候选事件时，执行器应在冲突结果中显式标记该结果需要二次确认（例如 `needs_confirmation=true`）
- 应用侧收到该指令后，仍需要向用户确认；若结果带有确认标记，应优先把它视为“批量删除候选”而不是普通失败
...
6. `delete_event` 至少应提供 `title` 或 `date`
